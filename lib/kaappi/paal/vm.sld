      ...
      (let ((cur-in  (%paal-cell-parameter (current-input-port)))
        ...
        (map (lambda (pair) (cons (car pair) (vector (cdr pair))))
           `(;; Arithmetic
             ...
             (list-copy . ,list-copy)
             (assq . ,assq) (assv . ,assv)
             ;; The three-argument forms take a comparison this VM must
             ;; call through the trampoline; the two-argument ones stay
             ;; the host's.  See the same pair in the bytecode blob.
             (assoc . ,(lambda (x alist . rest)
                         (if (null? rest)
                             (assoc x alist)
                             (let ((same? (car rest)))
                               (let loop ((l alist))
                                 (cond ((null? l) #f)
                                       ((trampoline (same? x (car (car l))))
                                        (car l))
                                       (else (loop (cdr l)))))))))
             (memq . ,memq) (memv . ,memv)
             (member . ,(lambda (x lst . rest)
                          (if (null? rest)
                              (member x lst)
                              (let ((same? (car rest)))
                                (let loop ((l lst))
                                  (cond ((null? l) #f)
                                        ((trampoline (same? x (car l))) l)
                                        (else (loop (cdr l)))))))))
