...
(define-library (srfi 44)
  ...
  (begin
    ...
    ;; [paal adaptation] The record kinds are asked before `vector?`.
    ;; kaappi's records are opaque, so either order answers the same there;
    ;; paal's are tagged vectors, and asking `vector?` first reported every
    ;; map, table, set and bag as 'vector.  Same representation leak the
    ;; (srfi 128) default ordering records.
    (define (%kind obj)
      (cond
        ((list? obj) 'list)
        ((string? obj) 'string)
