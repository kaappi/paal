...
(define-library (srfi 164)
  ...
  (begin
    ...
    ;; [paal adaptation] `array?` is asked before `vector?`, as in the
    ;; (srfi 25) copy of this dispatch: paal's records are tagged vectors,
    ;; so the `vector?` arm claimed every packed index *array* and read its
    ;; record slots as indices.  kaappi's records are opaque and answer the
    ;; same either way.
