...
;; [paal adaptation] (scheme char) and two (srfi 13) names added to the
;; import: the file uses string-upcase, string-reverse and
;; string-titlecase without importing them, which kaappi's unenforced
;; import scope tolerates and paal's conformance check rejects.  The
;; SRFI 13 pair comes in through `only` because the whole library would
;; also bring its own string-upcase, and importing one name from two
;; libraries is an error under R7RS 5.2.
(import (scheme char) (only (srfi 13) string-reverse string-titlecase)
        (scheme base) (scheme write) (scheme process-context) (srfi 54) (srfi 64))
