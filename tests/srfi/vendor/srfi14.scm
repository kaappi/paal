...
;; [paal adaptation] (scheme cxr) added: the file uses caddr without
;; importing it — same unenforced-import class as the srfi 18 note below.
(import (scheme base) (scheme char) (scheme cxr) (scheme process-context)
        (srfi 64) (srfi 14))
...
;; [paal adaptation] the one SRFI 18 test is excised and the (srfi 18)
;; import dropped: it probes kaappi's per-thread-heap sharing of charset
;; constants, which has no analogue here, and SRFI 18 is recorded as out
;; of scope for paal (docs/TODO.md "Not planned").
(test-equal "a constant is still sound when read twice"
  (list 136726 136726)
  (list (char-set-size char-set:letter) (char-set-size char-set:letter)))
...
;; [paal adaptation] 200 trials → 8: the brute-force model is quadratic in
;; code points per trial, tolerable natively and minutes through paal's
;; in-process pipeline.  The property stays exercised; the volume does not.
(test-equal "8 randomized trials agree with a brute-force set model"
  '() (model-mismatches 8))
